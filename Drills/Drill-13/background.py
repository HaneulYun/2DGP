...
class InfiniteBackground:
    ...
    def update(self):
        ...
        # quadrant 2
        self.q2l = 0  # (int(self.center_object.x) - self.canvas_width // 2) % self.w
        self.q2b = 0  # int(self.center_object.y) % self.h
        self.q2w = 0  # clamp(0, self.w - self.q2l, self.w)
        self.q2h = 0  # clamp(0, self.h - self.q2b, self.h)

        # quadrand 4
        self.q4l = 0  #int(self.center_object.x) % self.w
        self.q4b = 0  #(int(self.center_object.y) - self.canvas_height // 2) % self.h
        self.q4w = 0  #clamp(0, self.w - self.q4l, self.w)
        self.q4h = 0  #clamp(0, self.h - self.q4b, self.h)

        # quadrand 1
        self.q1l = 0  #int(self.center_object.x) % self.w
        self.q1b = 0  #int(self.center_object.y) % self.h
        self.q1w = 0  #clamp(0, self.w - self.q1l, self.w)
        self.q1h = 0  #clamp(0, self.h - self.q1b, self.h)
